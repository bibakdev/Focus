...
const targetsPath = path.join(__dirname, 'consistencyWeeklyTargets.json');
const resultsPath = path.join(__dirname, 'consistencyWeeklyResults.json');
...
function showTargets() {
  ...
  try {
    ...
    console.log(chalk.red('❌ consistencyWeeklyTargets.json خراب است.'));
    return;
  }

  console.log(chalk.bold(`\n🎯 Weekly Consistency Challenge Targets 🔄: \n`));
...
function showSummary() {
  ...
  console.log(chalk.bold('\n📈 Weekly Consistency Challenge Status:\n'));
...
Available commands:
  ...
  if (results) {
    ...
    if (stored) {
      ...
      console.log(
        chalk.bold(
          `\n🔄 Weekly Consistency Challenge Results: (${daysPassed}d => ${totalDays}d)\n`
        )
      );

      // ✅ مرتب‌سازی: اول پاس‌ها، بعد ناپاس‌ها
      // داخل هر گروه، مرتب‌سازی براساس target از زیاد به کم
      updatedResults.sort((a, b) => {
        // اول پاس‌دارها از ناپاس‌ها جدا بشن
        if (a.pass !== b.pass) return a.pass ? -1 : 1;

        // اگر هر دو پاس یا هر دو ناپاس بودن → sort بر اساس target descending
      ...
      if (daysPassed === totalDays) {
        ...
        console.log(chalk.bold('👑Champions of Weekly Consistency:\n'));
        console.log(`🥇 → ${golden.length ? golden.join(' - ') : '---'}`);
        console.log(`🥈 → ${silver.length ? silver.join(' - ') : '---'}`);
        console.log(`🥉 → ${bronze.length ? bronze.join(' - ') : '---'}`);

        // 🔁 چاپ جداگانه Golden Consistency به سبک جدید
        const goldenUsers = Object.entries(data)
          .filter(
            ([name, info]) =>
              name !== '_daysPassed' && info.count >= GOLDEN_DAYS
          )
          .map(([name]) => name);

        if (goldenUsers.length > 0) {
          console.log(chalk.bold(`\n🔁🥇${goldenUsers.join('-')}`));
        } else {
          console.log(chalk.gray('\n😢 هیچ‌کس جزو Golden Consistency نیست.'));
        }
      }
