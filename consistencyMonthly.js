...
const targetsPath = path.join(__dirname, 'consistencyMonthlyTargets.json');
const resultsPath = path.join(__dirname, 'consistencyMonthlyResults.json');
...
function showTargets() {
  ...
  try {
    ...
    console.log(chalk.red('❌ consistencyMonthlyTargets.json خراب است.'));
    return;
  }

  console.log(chalk.bold(`\n🎯 Monthly Consistency Challenge Targets 🔄: \n`));
...
function showSummary() {
  ...
  console.log(chalk.bold('\n📈 Monthly Consistency Challenge Status:\n'));
...
Available commands:
  ...
  if (results) {
    ...
    if (stored) {
      ...
      console.log(
        chalk.bold(
          `\n🔄 Monthly Consistency Challenge Results: (${daysPassed}d => ${totalDays}d)\n`
        )
      );
      ...
      if (daysPassed === totalDays) {
        ...
        const rankings = Object.entries(data)
          .filter(([name]) => name !== '_daysPassed')
          .map(([name, info]) => ({ name, days: info.count }))
          .filter((u) => u.days >= MIN_DAYS) // فقط کسانی که حداقل 21 روز
          .sort((a, b) => b.days - a.days); // مرتب‌سازی از بیشترین به کمترین
        ...
        const pushRank = (symbol) => {
          if (rankGroup.length) {
            outputRanks.push(`${symbol}${rankGroup.join('-')}`);
            rankGroup = [];
          }
        };

        for (let i = 0; i < rankings.length; i++) {
          const { name, days } = rankings[i];

          // اگر تعداد روز تغییر کرد → رتبه قبلی را چاپ کن
          if (prevDays !== null && days !== prevDays) {
            if (rankNum === 1) pushRank('🏆');
            else if (rankNum === 2) pushRank('🥈');
            else if (rankNum === 3) pushRank('🥉');
            else pushRank(`${rankNum}.`);
            rankNum++;
          }

          rankGroup.push(`${name}(${days}d)`);
          prevDays = days;
        }
        ...
        console.log(
          chalk.bold('\n📊 Monthly Consistency Challenge Ranking:\n')
        );
        console.log(outputRanks.join('\n'));
      }
